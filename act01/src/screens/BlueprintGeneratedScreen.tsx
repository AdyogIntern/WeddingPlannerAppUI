...
import { Sparkles, CheckSquare, Share2, Layers, ArrowRight, ChevronLeft } from 'lucide-react';

export const BlueprintGeneratedScreen: React.FC = () => {
  const { setScreen, weddingScore, getTotalCostINR, getTotalCostInCurrency, currency, getFilledSlotsCount } = useWeddingStore();
  ...
  return (
    ...
    >
      {/* Top Burgundy Header */}
      <div className="bg-[#671B2B] text-white px-5 pt-3.5 pb-4 shrink-0">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-[#C9A227] text-[10px] font-bold tracking-widest uppercase">
            YOUR BLUEPRINT IS READY
          </div>
        </div>
        <h1 className="text-xl font-serif font-bold leading-tight text-white">
          Three days, five functions, 420 guests.
        </h1>
      </div>

      {/* Main Scrollable Body with Light Background */}
      <View style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', gap: '14px' }}>
        
        {/* Main Stats Grid */}
        <div className="grid grid-cols-2 gap-3">
          {/* Estimated Cost Stat Card */}
        ...
        <div className="bg-white rounded-2xl p-4 border border-[#E5E0D8] shadow-2xs space-y-3 text-[#1D1D1F]">
          <h4 className="text-sm font-serif font-bold text-gray-900 pb-0.5">
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <span className="text-xs font-semibold text-gray-900 leading-tight block">
                Lock the Muhurtham venue — availability moves fastest
              </span>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <span className="text-xs font-semibold text-gray-900 leading-tight block">
                Confirm your guest count per function
              </span>
      ...
      <View style={{ padding: '10px 16px 16px', shrink: 0, alignItems: 'center', backgroundColor: '#FAF8F5' }}>
        <button
        ...
        <button 
          ...
          className="mt-2 text-[11px] text-[#8C7A6B] hover:text-gray-900 transition-colors cursor-pointer bg-transparent border-none outline-none font-medium"
        >
