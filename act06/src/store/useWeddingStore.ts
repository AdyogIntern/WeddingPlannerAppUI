...
export const useWeddingStore = create<WeddingStoreState>((set, get) => ({
  ...
  setQuestionIndex: (index) => set({ questionIndex: Math.max(0, Math.min(9, index)) }),
  ...
  nextQuestion: () => {
    ...
    if (questionIndex < 9) {
      ...
      // Finished 10 questions -> Go to Screen 5 (Generating Blueprint)
