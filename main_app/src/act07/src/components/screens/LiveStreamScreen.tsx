...
export const LiveStreamScreen: React.FC<Props> = ({ onBack, onNavigateNext }) => {
      ...
      <div className="bg-[#1E1815] text-white p-5 pt-6">
        <div className="flex justify-between items-center mb-3">
          {onBack && (
            <button 
              onClick={onBack}
              className="text-[#FAF7F2] bg-white/10 hover:bg-white/20 px-3 py-1 rounded-lg text-xs transition-colors cursor-pointer"
            >
              ← Back
            </button>
          )}
          {onNavigateNext && (
            <button 
              ...
              className="text-[#FAF7F2] bg-[#7A2232] hover:bg-[#8d2a3c] px-3 py-1 rounded-lg text-xs transition-colors cursor-pointer ml-auto"
            >
              Next →
            </button>
          )}
