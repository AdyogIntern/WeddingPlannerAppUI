...
export const MobileContainer: React.FC = () => {
  ...
  const renderActiveScreen = () => {
    switch (activeScreen) {
      ...
      case 'live_stream':
        return <LiveStreamScreen onNavigateNext={() => navigateTo('invitation')} />;
      case 'invitation':
        return (
          <InvitationScreen
  ...
  return (
    <div className="min-h-screen bg-[#F0ECE1] text-[#1F1A17] flex flex-col items-center justify-start py-0 sm:py-4 px-0 sm:px-4">
      ...
      <div className="w-full max-w-md bg-[#FAF7F2] min-h-screen sm:min-h-[810px] sm:rounded-2xl shadow-xl border border-[#E0D8CC] flex flex-col overflow-hidden relative">
        {/* Top Header Bar with Back Button and Step Info */}
        <div className="bg-[#FAF7F2] border-b border-[#E8E0D5] px-3.5 py-2.5 flex items-center justify-between sticky top-0 z-50 shrink-0">
          {(activeScreen !== 'auspicious_dates' || history.length > 0) ? (
            <button
              onClick={goBack}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-[#E8E0D5] text-[#7A2232] font-bold text-[12.5px] hover:bg-[#FAF0EC] active:scale-95 transition-all cursor-pointer shadow-2xs"
            >
              <ArrowLeft className="w-4 h-4 stroke-[2.5]" />
              <span>Back</span>
            </button>
          ) : (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#FAF0EC] border border-[#7A2232]/20 text-[#7A2232] font-bold text-[11px]">
              <span>Step 1 of 13</span>
            </div>
          )}

          <div className="flex flex-col items-end">
            <span className="text-[10px] font-bold tracking-wider uppercase text-[#7A2232] bg-[#FAF0EC] px-2 py-0.5 rounded-full border border-[#7A2232]/20">
              Step {currentStep} of 13
            </span>
            <span className="text-[12px] font-bold text-[#1F1A17] truncate max-w-[210px] mt-0.5">
